        ...
        : base(resource)
    {
        ...
        _propertySourceNodes = new Lazy<List<(string Name, Lazy<IEnumerable<ISourceNode>> Node)>>(() =>
        {
            ...
            foreach (PropertyDescriptor prop in TypeDescriptor.GetProperties(Resource))
            {
                ...
                {
                    list.Add((propName, new Lazy<IEnumerable<ISourceNode>>(() => [new FhirStringSourceNode(() => PrimitiveTypeConverter.ConvertTo<string>(thisProp.GetValue(Resource)), propName, $"{Location}.{propName}")])));
                }
            }
